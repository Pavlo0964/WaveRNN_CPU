...
class LSA(nn.Module):
    ...
    def init_attention(self, encoder_seq_proj) :
        ...
        self.cumulative = torch.zeros(b, t).cpu()
        self.attention = torch.zeros(b, t).cpu()
...
class Decoder(nn.Module) :
    ...
    def zoneout(self, prev, current, p=0.1) :
        mask = torch.zeros(prev.size()).bernoulli_(p).cpu()
...
class Tacotron(nn.Module) :
    ...
    def forward(self, x, m, generate_gta=False) :
        ...
        attn_hidden = torch.zeros(batch_size, self.decoder_dims).cpu()
        rnn1_hidden = torch.zeros(batch_size, self.lstm_dims).cpu()
        rnn2_hidden = torch.zeros(batch_size, self.lstm_dims).cpu()
        hidden_states = (attn_hidden, rnn1_hidden, rnn2_hidden)
        
        # Initialise all lstm cell states and pack into tuple
        rnn1_cell = torch.zeros(batch_size, self.lstm_dims).cpu()
        rnn2_cell = torch.zeros(batch_size, self.lstm_dims).cpu()
        cell_states = (rnn1_cell, rnn2_cell)
        
        # <GO> Frame for start of decoder loop
        go_frame = torch.zeros(batch_size, self.n_mels).cpu()
        
        # Need an initial context vector
        context_vec = torch.zeros(batch_size, self.decoder_dims).cpu()
    ...
    def generate(self, x, steps=2000) :
        ...
        batch_size = 1
        x = torch.LongTensor(x).unsqueeze(0).cpu()
    ...
    def load(self, path):
        self.load_state_dict(torch.load(path, map_location=lambda storage, loc: storage), strict=False)
